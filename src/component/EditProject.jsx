...
const EditProject = () => {
    const { targetProjectId } = useParams();
    ...
    // Fetch project data and populate form fields
    useEffect(() => {
        const targetProject = projects.find((pro) => pro._id === targetProjectId);
        ...
        if (targetProject) {
            ...
            // Set selected skills
            if (Array.isArray(targetProject.skills)) {
                const selectedSkillObjects = targetProject.skills.map(skillName => {
            ...
            const memberObj = members.find(m => m.name === targetProject.members);
            setSelectedMembers(memberObj || null);

            // Set isActive status
            setChecked(targetProject.isActive || false);
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.patch(
                `${baseURL}/project/edit/${targetProjectId}`,
                {
                    projectName,
                    description,
                    skills: skillNames,
    ...
    return (
        <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 md:px-8 py-6 bg-sky-50 rounded-lg shadow-md my-8">
            <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-sky-800">Edit Project</h2>
